...
func TestContractStore(t *testing.T) {
	...
	err = c.ParseABI()
	assert.NoError(t, err, "Parse query result ABI should not throw exception")
	assert.Equal(t, 22, len(c.Methods), "query result ABI should contain %d methods", 22)
	assert.Equal(t, 3, len(c.Events), "query result ABI should contain %d events", 3)
}
